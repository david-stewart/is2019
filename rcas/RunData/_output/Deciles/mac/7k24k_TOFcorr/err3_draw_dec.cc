// draw the errors, but with the percent errors between 0k7k_raw and 0k7k_TOFcorr
// take out the error from NchCorr for now
#include <TPad.h>
#include <TH1D.h>
#include <TH2D.h>
#include <TProfile.h>
...
void err3_draw_dec(const char* tag) {
    ...
    vector<const char*> titles{"Minimum Bias Events",
            "Trigger E_{T}#in (4, 8) GeV",
            "Trigger E_{T}#in (8,12) GeV",
            "Trigger  E_{T}> 12 GeV"};

    /* TCanvas *c = new TCanvas("c", "",10,45,946,968); */
   TCanvas *c = new TCanvas("c", "",10,88,1022,844);
    /* TCanvas *c = new TCanvas("c", "",10,45,946,968); */
    c->SetFillStyle(4000);
    /* c->Divide(1,4,0.1,0); */
    ...
    TFile* f_out = new TFile(Form("err3_decile_%s.root",tag),"recreate");
    ...
    double y_lo = 1e-9;
    double y_hi = 4e-5;
    TPad *c_1 = new TPad("c_1", "_1",0,0.80,0.9,0.98);
   c_1->Draw();
   c_1->cd();
   c_1->Range(-26250.5,-9.045757,104999.5,-4.30103);
   c_1->SetFillColor(0);
   c_1->SetBorderMode(0);
   c_1->SetBorderSize(2);
   c_1->SetLogy();
   c_1->SetLeftMargin(0.2);
   c_1->SetRightMargin(0);
   c_1->SetTopMargin(0);
   ...
   c_1->SetFrameBorderMode(0);
    ...
    SetYAxis(HT_dist,"#frac{1}{N_{ev}}  #frac{dN_{ev}}{dEA_{BBC}}",y_lo, y_hi);

    // axis etc...
   HT_dist->SetMarkerSize(0.9);
   HT_dist->GetXaxis()->SetTitle("BBC");
   HT_dist->GetXaxis()->SetRange(1,105000);
   HT_dist->GetXaxis()->SetTitleOffset(1.2);
   HT_dist->GetYaxis()->SetTitle("#frac{1}{N_{ev}}  #frac{dN_{ev}}{dEA_{BBC}}");
   HT_dist->GetYaxis()->SetLabelFont(42);
   HT_dist->GetYaxis()->SetLabelOffset(-0.006);
   HT_dist->GetYaxis()->SetLabelSize(0.13);
   HT_dist->GetYaxis()->SetTitleSize(0.13);
   HT_dist->GetYaxis()->SetTitleOffset(0.32);
   HT_dist->GetYaxis()->SetTitleFont(42);
   HT_dist->GetZaxis()->SetLabelFont(42);
   HT_dist->GetZaxis()->SetLabelSize(0.035);
   HT_dist->GetZaxis()->SetTitleSize(0.035);
   HT_dist->GetZaxis()->SetTitleOffset(1);
   HT_dist->GetZaxis()->SetTitleFont(42);
   HT_dist->GetYaxis()->SetRangeUser(9e-10,5e-5);
   //----
   // draw the decile lines...
   //----
    HT_dist->Draw("PE");
   ...
   /* TLegend *leg = new TLegend(0.291498,0.04968383,0.6891588,0.757302,NULL,"brNDC"); */
   TLegend *leg = new TLegend(0.624183,0.05969339,0.8899782,0.3988604,NULL,"brNDC");
    ...
    leg->Draw("same");
    TAxis *edges = fGetObject<TH2D*>(f_004, "h2_bbcDec_C", "nEvents_C_0")->ProjectionX()->GetXaxis();
    ...
    SetYAxis(nEvents_C[0], "#frac{1}{N_{ev}}  #frac{dN_{ev}}{dEA_{BBC}}", -0.04, 0.04, 0.018);
    //font etc...
    for (int i=0;i<4;++i) nEvents_C[i]->Write();

    /* leg = new TLegend(0.291498,0.04968383,0.6891588,0.757302,NULL,"brNDC"); */
    //-----------------------
    //  TPad 3 - nch
    //-----------------------
    ...
    //-----------------------
    c->cd();
    TPad *c_3 = new TPad("c_3", "_3",0,0.35,.9,0.60);
   c_3->Draw();
   c_3->cd();
   c_3->Range(-26250,5.5,105000,28);
   c_3->SetFillColor(0);
   c_3->SetBorderMode(0);
   c_3->SetBorderSize(2);
   c_3->SetLeftMargin(0.2);
   c_3->SetRightMargin(0);
   c_3->SetTopMargin(0);
   c_3->SetBottomMargin(0);
   ...
   c_3->SetFrameBorderMode(0);
    ...
    // Get the nch error from k7k_TOF correction
    /* TFile* f_0k7k = new TFile("../0k7k_TOFcorr/decile_0k7k_opt3.root","read"); */
    TFile* f_0k7k = new TFile("../0k7k_raw/decile_0k7k_opt0.root","read");
    vector<TH1D*> nch_0k7k_TOFcorr;
    for (int i{0};i<4;++i) {
        TH1D* get;
        f_0k7k->GetObject(Form("nch_dAu_%i",i),get);
        get->SetName(Form("nch_0k7k_TOFcorr_%i",i));
        cout << get->GetName() << endl;
        nch_0k7k_TOFcorr.push_back(get);
    }
    // Get the nch error from 0k7k_raw
    TFile* f_raw = new TFile("../0k7k_raw/decile_0k7k_opt0.root","read");
    vector<TH1D*> nch_raw;
    for (int i{0};i<4;++i) {
        TH1D* get;
        f_0k7k->GetObject(Form("nch_dAu_%i",i),get);
        get->SetName(Form("nch_raw_%i",i));
        cout << get->GetName() << endl;
        nch_raw.push_back(get);
    }

    // Get the nch error from correcting with subtracting relgar tracks
    TFile* f_NchCorr = new TFile("../7k24k_NchCorr/decile_7k24k_opt1.root","read");
    vector<TH1D*> nch_NchCorr;
    for (int i{0};i<4;++i) {
        TH1D* get;
        f_NchCorr->GetObject(Form("nch_dAu_%i",i),get);
        get->SetName(Form("nch_NchCorr_%i",i));
        nch_NchCorr.push_back(get);
    }
    ...
    // etc...
   nch_dAu[0]->GetXaxis()->SetTitle("BBC");
   nch_dAu[0]->GetXaxis()->SetLabelFont(42);
   nch_dAu[0]->GetXaxis()->SetLabelSize(0.035);
   nch_dAu[0]->GetXaxis()->SetTitleSize(0.035);
   nch_dAu[0]->GetXaxis()->SetTitleOffset(1);
   nch_dAu[0]->GetXaxis()->SetTitleFont(42);
   nch_dAu[0]->GetYaxis()->SetTitle("#LTN_{ch}#GT");
   nch_dAu[0]->GetYaxis()->SetLabelFont(42);
   nch_dAu[0]->GetYaxis()->SetLabelOffset(0.007);
   nch_dAu[0]->GetYaxis()->SetLabelSize(0.1);
   nch_dAu[0]->GetYaxis()->SetTitleSize(0.1);
   nch_dAu[0]->GetYaxis()->SetTitleOffset(0.38);
   nch_dAu[0]->GetYaxis()->SetTitleFont(42);
   nch_dAu[0]->GetZaxis()->SetLabelFont(42);
   nch_dAu[0]->GetZaxis()->SetLabelSize(0.035);
   nch_dAu[0]->GetZaxis()->SetTitleSize(0.035);
   nch_dAu[0]->GetZaxis()->SetTitleOffset(1);
   nch_dAu[0]->GetZaxis()->SetTitleFont(42);
   nch_dAu[0]->GetYaxis()->SetRangeUser(5.5,28);
    ...
    for (int i{0};i<3;++i) {
		...
		//error from 0-7k
        nch_0k7k_TOFcorr[i]->Add(nch_dAu[i],-1);
        nch_raw[i]->Add(nch_dAu[i],-1);
        /* nch_raw[i]->Divide(nch_0k7k_TOFcorr[i]); */
        /* nch_raw[i]->Multiply(nch_dAu[i]); */
        nch_NchCorr[i]->Add(nch_dAu[i],-1);
        ...
        nch_0k7k_TOFcorr[i]->Multiply(nch_0k7k_TOFcorr[i]);
        nch_raw[i]->Multiply(nch_raw[i]);
        nch_NchCorr[i]->Multiply(nch_NchCorr[i]);
	
		nch_diff->Add(nch_clip[i]);
		nch_diff->Add(nch_pp[i]);
		/* nch_diff->Add(nch_0k7k_TOFcorr[i]); */
		/* nch_diff->Add(nch_NchCorr[i]); */
		nch_diff->Add(nch_raw[i]);
    ...
    myDrawLines(edges, 5.5,28);
    /* nch_dAu[3]->Draw("PE same"); */
    for (int i=0;i<4;++i) nch_dAu[i]->Write();

    //draw TPad 2
    c->cd();
    TPad *c_2 = new TPad("c_2", "_2",0,0.60,0.9,0.80);
   c_2->Draw();
   c_2->cd();
   c_2->Range(-26250,0.04,105000,0.18);
   c_2->SetFillColor(0);
   c_2->SetBorderMode(0);
   c_2->SetBorderSize(2);
   ...
   c_2->SetTopMargin(0);
   c_2->SetBottomMargin(0);
   c_2->SetFrameBorderMode(0);
   c_2->SetFrameBorderMode(0);
   nEvents_C[0]->GetXaxis()->SetTitle("BBC");
   nEvents_C[0]->GetXaxis()->SetRange(1,10);
   nEvents_C[0]->GetXaxis()->SetTitleOffset(1.2);
   /* nEvents_C[0]->GetYaxis()->SetTitle("#frac{1}{N_{ev}}  #frac{dN_{ev}}{dEA_{BBC}}"); */
   nEvents_C[0]->GetYaxis()->SetTitle("#frac{N_{ev}^{EA-bin}}{#Sigma_{EA-bin}N_{ev}^{EA-bin}}");
   nEvents_C[0]->GetYaxis()->SetLabelFont(42);
   nEvents_C[0]->GetYaxis()->SetLabelOffset(0.007);
   nEvents_C[0]->GetYaxis()->SetLabelSize(0.11);
   nEvents_C[0]->GetYaxis()->SetTitleSize(0.13);
   nEvents_C[0]->GetYaxis()->SetTitleOffset(0.35);
   nEvents_C[0]->GetYaxis()->SetTitleFont(42);
   nEvents_C[0]->GetZaxis()->SetLabelFont(42);
   nEvents_C[0]->GetZaxis()->SetLabelSize(0.035);
   nEvents_C[0]->GetZaxis()->SetTitleSize(0.035);
   nEvents_C[0]->GetZaxis()->SetTitleOffset(1);
   nEvents_C[0]->GetZaxis()->SetTitleFont(42);
   nEvents_C[0]->GetYaxis()->SetRangeUser(0.04,0.18);
   //---
    nEvents_C[0]->Draw("PE");
    ...
    myDrawLines(edges, 0.04,0.18);
   leg = new TLegend(0.6285403,0.05860806,0.9270153,0.4188034,NULL,"brNDC");
    leg->SetFillColor(4000);
    leg->SetLineColor(4000);
    vector<TH1D*> dummies2;
    for (int i=2;i<5;++i) {
        TString title;
        int color;
        int shape;
        ...
        shape = shapes[k];
        title = titles[k];
        dummies2.push_back(new TH1D(Form("dummy_%i",i),";;",1,0.,1.));
        SetMCS(dummies2[i-2], shape, color);
        leg->AddEntry(dummies2[i-2], title.Data());
    }
    leg->Draw("same");
    ...
    //-----------------------
    c->cd();
    TPad *c_4 = new TPad("c_3", "_3",0,0.0,.9,0.35);
   c_4->Draw();
   c_4->cd();
   c_4->Range(-26250,-4.760563,105000,22);
   c_4->SetFillColor(0);
   c_4->SetBorderMode(0);
   c_4->SetBorderSize(2);
   c_4->SetLeftMargin(0.2);
   c_4->SetRightMargin(0);
   c_4->SetTopMargin(0);
   c_4->SetBottomMargin(0.29);
   ...
   c_4->SetFrameBorderMode(0);
    ...
    // get errors from at 0k7
    vector<TH1D*> sumpt_0k7k_TOFcorr;
    for (int i{0};i<4;++i) {
        TH1D* get;
        f_0k7k->GetObject(Form("sumpt_dAu_%i",i),get);
        get->SetName(Form("sumpt_0k7k_TOFcorr_%i",i));
        cout << get->GetName() << endl;
        sumpt_0k7k_TOFcorr.push_back(get);
    }
    /* TFile* f_raw = new TFile("../0k7k_raw/decile_0k7k_opt0.root","read"); */
    vector<TH1D*> sumpt_raw;
    for (int i{0};i<4;++i) {
        TH1D* get;
        f_0k7k->GetObject(Form("sumpt_dAu_%i",i),get);
        get->SetName(Form("sumpt_raw_%i",i));
        cout << get->GetName() << endl;
        sumpt_raw.push_back(get);
    }
    vector<TH1D*> sumpt_NchCorr;
    for (int i{0};i<4;++i) {
        TH1D* get;
        f_NchCorr->GetObject(Form("sumpt_dAu_%i",i),get);
        get->SetName(Form("sumpt_NchCorr_%i",i));
        cout << get->GetName() << endl;
        sumpt_NchCorr.push_back(get);
    }

    //etc....
   sumpt_dAu[0]->SetStats(0);
   sumpt_dAu[0]->GetXaxis()->SetTitle("Event Activity_{BBC}");
   sumpt_dAu[0]->GetXaxis()->SetLabelFont(42);
   sumpt_dAu[0]->GetXaxis()->SetLabelOffset(0.007);
   sumpt_dAu[0]->GetXaxis()->SetLabelSize(0.08);
   sumpt_dAu[0]->GetXaxis()->SetTitleSize(0.08);
   sumpt_dAu[0]->GetXaxis()->SetTitleOffset(1);
   sumpt_dAu[0]->GetXaxis()->SetTitleFont(42);
   sumpt_dAu[0]->GetYaxis()->SetTitle("#LT#Sigma #it{p}_{T}^{ch}#GT (GeV/c)");
   sumpt_dAu[0]->GetYaxis()->SetLabelFont(42);
   sumpt_dAu[0]->GetYaxis()->SetLabelOffset(0.007);
   sumpt_dAu[0]->GetYaxis()->SetLabelSize(0.07);
   sumpt_dAu[0]->GetYaxis()->SetTitleSize(0.08);
   sumpt_dAu[0]->GetYaxis()->SetTitleOffset(0.46);
   sumpt_dAu[0]->GetYaxis()->SetTitleFont(42);
   sumpt_dAu[0]->GetZaxis()->SetLabelFont(42);
   sumpt_dAu[0]->GetZaxis()->SetLabelSize(0.035);
   sumpt_dAu[0]->GetZaxis()->SetTitleSize(0.035);
   sumpt_dAu[0]->GetZaxis()->SetTitleOffset(1);
   sumpt_dAu[0]->GetZaxis()->SetTitleFont(42);
   sumpt_dAu[0]->GetYaxis()->SetRangeUser(3,22);
   sumpt_dAu[0]->Draw("PE");
    ...
    for (int i{0};i<3;++i) {
        ...
        //
		//error from 0-7k
        sumpt_0k7k_TOFcorr[i]->Add(sumpt_dAu[i],-1);
        sumpt_raw[i]->Add(sumpt_dAu[i],-1);

        sumpt_NchCorr[i]->Add(sumpt_dAu[i],-1);
        ...
        sumpt_0k7k_TOFcorr[i]->Multiply(sumpt_0k7k_TOFcorr[i]);
        sumpt_raw[i]->Multiply(sumpt_raw[i]);
        sumpt_NchCorr[i]->Multiply(sumpt_NchCorr[i]);
	
		sumpt_diff->Add(sumpt_clip[i]);
		sumpt_diff->Add(sumpt_pp[i]);
		/* sumpt_diff->Add(sumpt_0k7k_TOFcorr[i]); */
		sumpt_diff->Add(sumpt_raw[i]);
		/* sumpt_diff->Add(sumpt_NchCorr[i]); */
    ...
    myDrawLines(edges, 3,22);
    /* sumpt_dAu[3]->Draw("PE same"); */
    for (int i=0;i<4;++i) sumpt_dAu[i]->Write();


    // cut it off at TPad 4! 
    // TPad 5 will be separate
    ...
    //     errors from difference in _orig and _corr
    //-----------------------
   TCanvas *canv_pt = new TCanvas("canv_pt", "",10,45,1163,529);
   canv_pt->Range(-13125,-0.0478136,118238.1,4.346909);
   canv_pt->SetFillColor(0);
   canv_pt->SetBorderMode(0);
   canv_pt->SetBorderSize(2);
   canv_pt->SetBottomMargin(0.1533865);
   canv_pt->SetFrameBorderMode(0);
   canv_pt->SetFrameBorderMode(0);
    /* TCanvas *canv_pt = new TCanvas("canv_pt"); */
    canv_pt->Draw();
    canv_pt->cd();
    ...
    maxpt_C[0]->GetXaxis()->SetLabelFont(42);
    maxpt_C[0]->GetXaxis()->SetLabelOffset(0.007);
    maxpt_C[0]->GetXaxis()->SetLabelSize(0.05);
    maxpt_C[0]->GetXaxis()->SetTitleSize(0.05);
    maxpt_C[0]->GetXaxis()->SetTitleOffset(1.05);
    maxpt_C[0]->GetXaxis()->SetTitleFont(42);
    maxpt_C[0]->GetYaxis()->SetTitle("#LTmax(#it{p}_{T}^{ch})#GT (GeV/c)");
    maxpt_C[0]->GetYaxis()->SetNdivisions(509);
    maxpt_C[0]->GetYaxis()->SetLabelFont(42);
    maxpt_C[0]->GetYaxis()->SetLabelOffset(0.007);
    maxpt_C[0]->GetYaxis()->SetLabelSize(0.05);
    maxpt_C[0]->GetYaxis()->SetTitleSize(0.05);
    maxpt_C[0]->GetYaxis()->SetTitleOffset(0.62);
    maxpt_C[0]->GetYaxis()->SetTitleFont(42);
    maxpt_C[0]->GetZaxis()->SetLabelFont(42);
    maxpt_C[0]->GetZaxis()->SetLabelSize(0.035);
    maxpt_C[0]->GetZaxis()->SetTitleSize(0.035);
    maxpt_C[0]->GetZaxis()->SetTitleOffset(1);
    maxpt_C[0]->GetZaxis()->SetTitleFont(42);
    maxpt_C[0]->GetYaxis()->SetRangeUser(0.4,2.9);
    maxpt_C[0]->SetStats(0);
    maxpt_C[0]->SetTitle("");
    maxpt_C[0]->Draw("PE");
    maxpt_C[0]->Write();
    ...
    leg = new TLegend(0.3781223,0.1924603,0.8914729,0.3492063,NULL,"brNDC");
    leg->SetFillColor(4000);
    leg->SetLineColor(4000);
    leg->AddEntry(maxpt_C[0], titles[0]);
    leg->AddEntry(maxpt_C[1], titles[1]);
    leg->AddEntry(maxpt_C[2], titles[2]);
    leg->Draw("same");
    /* maxpt_C[3]->Draw("PE same"); */
    maxpt_C[1]->Write();
    maxpt_C[2]->Write();
    maxpt_C[3]->Write();
    myDrawLines(edges, 0.4,2.9);

    c->SaveAs(Form("../pdf/err3_%s.pdf",tag));
    canv_pt->SaveAs(Form("../pdf/maxpt_%s.pdf",tag));
    f_out->Save();
