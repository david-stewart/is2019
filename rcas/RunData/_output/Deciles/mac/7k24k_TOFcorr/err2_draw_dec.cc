...
void err2_draw_dec(const char* tag) {
    ...
    TFile* f_out = new TFile(Form("err2_decile_%s.root",tag),"recreate");
    ...
    SetYAxis(nch_dAu[0],"#frac{2}{2#pi} #frac{d^3#LTN_{ch}#GT}{dEA_{BBC}d#etad#phi",5,32,0.007);
    nch_dAu[0]->Draw("PE");
    ...
    c->SaveAs(Form("../pdf/err2_%s.pdf",tag));
    f_out->Save();
