...
// Fixed size dimensions of array or collections stored in the TTree if any.
 ...
 //JetEvent        *events;
   ...
   JetLoop(const char* name, double cutLo, double cutHi, TTree *tree=0);
   virtual ~JetLoop();
...
#ifdef JetLoop_cxx
JetLoop::JetLoop(const char* _name, double _cutLo, double _cutHi, TTree *tree) :
    name{_name}, cutLo{_cutLo}, cutHi{_cutHi}, fChain(0) 
{

// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../206_JetTree.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../206_JetTree.root");
      }
...
void JetLoop::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("bbc", &bbc, &b_events_bbc);
   fChain->SetBranchAddress("bbc_corr", &bbc_corr, &b_events_bbc_corr);
